...
# import time
# from .fodn_code import HaarWaveletTransform, fracOrdUU

# def run_fodn_analysis(data, chunk_size=1.0, numFract=50, niter=10, lambdaUse=0.5, base_time=0.0):
#     """
#     Processes the input data chunk by chunk without averaging.
#     For each complete chunk (of length chunk_size seconds), this function creates a new
#     instance of fracOrdUU, calls its fit() method on that chunk, and returns the raw output.
    
#     Parameters:
#       data: 2D numpy array (channels, timepoints)
#       chunk_size: duration (in seconds) of each chunk (e.g., 1.0)
#       numFract: parameter for fracOrdUU (e.g., 50)
#       niter: number of iterations for fracOrdUU (e.g., 10)
#       lambdaUse: regularization parameter (e.g., 0.5)
#       base_time: the absolute start time (in seconds) of the data passed in.
      
#     Returns:
#       A list of dictionaries, one per complete chunk. Each dictionary contains:
#          "alpha": fractional-order exponents (per channel) for that chunk,
#          "coupling_matrix": the estimated coupling matrix from the final iteration,
#          "chunk_start": absolute start time for the chunk,
#          "chunk_end": absolute end time for the chunk.
#          (No averaging is done across chunks.)
#     """
#     fs = 1000  # Assumed sampling rate
#     samples_per_chunk = int(chunk_size * fs)
#     num_chunks = data.shape[1] // samples_per_chunk
...
#     for i in range(num_chunks):
#         chunk_data = data[:, i * samples_per_chunk : (i+1) * samples_per_chunk]
#         model = fracOrdUU(numFract=numFract, niter=niter, lambdaUse=lambdaUse, verbose=1)
#         model._numCh = chunk_data.shape[0]
#         model._K = chunk_data.shape[1]
#         model.fit(chunk_data)
#         chunk_result = {
#             "alpha": model._order,
#             "coupling_matrix": model._AMat[-1, :, :],   # -1 index retrives last estimated coupling matrix
#             "chunk_start": base_time + i * chunk_size,
#             "chunk_end": base_time + (i+1) * chunk_size
#         }
