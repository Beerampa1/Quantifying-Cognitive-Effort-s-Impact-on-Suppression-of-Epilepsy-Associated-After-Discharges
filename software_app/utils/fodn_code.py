...
#############################################################################################
    def _getFractionalOrder(self, x):
        ...
        for i in range(numScales - 1):
            y = W[i, :j]
            variance = np.var(y, ddof=1)  # unbiased estimate

            #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            # Avoid log(0) by setting a small epsilon if variance is zero or negative<<<<<<<<<<<<<<<<<<<<<
            if variance <= 0:
                variance = 1e-10
            #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
############################################################################################
    def _performLeastSq(self, Y, X):
        ...
        #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        reg = 1e-8 * np.eye(XUse.shape[1])  # Regularization term to avoid singularity -<<<<<<<<<<<<<<<<<<<<<<
        ...
        for k in range(MAX_ITER):
            ...
            if self._performSparseComputation:
                if m >= n:
                    ...
                    x = q / rho - np.matmul(A.T, sLA.inv(UInv) * (sLA.inv(LInv) * np.matmul(A, q))) / rho ** 2
            else:
                if m >= n:
                    x = np.matmul(UInv, np.matmul(LInv, q))
                else:
                    x = q / rho - np.matmul(A.T, np.matmul(LA.inv(UInv), np.matmul(LA.inv(LInv), np.matmul(A, q)))) / rho ** 2
            ...
            if (history._r_norm[k] < history._eps_pri[k] and history._s_norm[k] < history._eps_dual[k]):
                break
        return np.squeeze(z)
