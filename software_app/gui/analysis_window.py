...
def rolling_statistic(data, window_size, func):
    """Compute rolling statistic using a sliding window of size 'window_size'."""
    if len(data) < window_size:
        return np.array([])
    return np.array([func(data[i:i+window_size]) for i in range(len(data) - window_size + 1)])
...
class AnalysisWindow(QDialog):
    def __init__(self, parent, signals, time_array, channel_indices, start_time, end_time, channel_names=None):
        super().__init__(parent)
    ...
    def initUI(self):
        ...
        self.orig_canvas = self.create_plot_canvas(self.t_snippet, self.snippets, "Original Data")
        ...
        self.feature_plot_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def create_plot_canvas(self, t, snippets_dict, title):
        ...
        i = 0
        for ch, data in snippets_dict.items():
            ch_name = self.channel_names[ch] if ch < len(self.channel_names) else f"Channel {ch}"
            ax.plot(t, data, label=ch_name, color=colors[i % len(colors)])
            i += 1
        ...
        canvas.mpl_connect("button_press_event", lambda event: self.expand_graph(title, t, snippets_dict) if event.dblclick else None)
        return canvas
    
    def expand_graph(self, title, t, snippets_dict):
        dlg = ExpandedGraphDialog(title, t, self.aggregate_snippets(snippets_dict), self)
        dlg.exec_()
    ...
    def add_feature_plot(self, title, t, feature_data):
        ...
        colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
        i = 0
        for ch, data in feature_data.items():
            ch_name = self.channel_names[ch] if ch < len(self.channel_names) else f"Channel {ch}"
            ax.plot(t, data, label=ch_name, color=colors[i % len(colors)])
            i += 1
        ax.set_xlabel("Time (s)")
        ...
        canvas = FigureCanvas(fig)
        canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        canvas.mpl_connect("button_press_event", lambda event: self.expand_graph(title, t, feature_data) if event.dblclick else None)
    ...
    def plot_feature(self, func, feature_title):
        ...
        for ch in self.channel_indices:
            ...
            feature_data[ch] = rolling_statistic(data, self.rolling_window, func)
        if not feature_data:
            QMessageBox.warning(self, "Error", "No data available for feature calculation.")
            return
        sample_len = len(next(iter(feature_data.values())))
        if sample_len == 0:
            QMessageBox.warning(self, "Insufficient Data", "Not enough data to compute rolling statistics.")
            return
    ...
    def plot_rms(self):
        """Plot rolling RMS (root mean square)."""
        def rms_func(x):
            return np.sqrt(np.mean(x**2))
        self.plot_feature(rms_func, "Rolling RMS")
    
    def save_orig_plot(self):
        """Save the original snippet figure."""
        fileName, _ = QFileDialog.getSaveFileName(
            self, "Save Original Plot", "original_plot.png", 
    ...
    def save_feature_plot(self):
        """Save the currently displayed feature plot."""
        if self.feature_plot_figure is None:
            QMessageBox.warning(self, "Error", "No feature plot available to save.")
            return
        fileName, _ = QFileDialog.getSaveFileName(
            self, "Save Feature Plot", "feature_plot.png", 
            "PNG Files (*.png);;JPEG Files (*.jpg)"
        )
        if fileName:
